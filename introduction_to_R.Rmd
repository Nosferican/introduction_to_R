...
df <- data.frame(table(modified_object = x))
ggplot(data = df, mapping = aes(x = modified_object, weight = Freq)) +
  geom_histogram(stat = "count") +
  labs(title = "Histogram of modified vector",
       x = "Value",
       y = "Count") +
...
```{r}
x <- as_factor(x = rep(x = letters[1:5], 10))
str(x)
x[10L]
levels(x = x)
```